...
from dmasif_encoder.helper import *
...
def soft_distances(x, y, batch_x, batch_y, num_atoms, smoothness=0.01, atomtypes=None):
...
def atoms_to_points_normals(
    ...
    with torch.enable_grad():
        ...
        dists = soft_distances(
            atoms, z, batch_atoms, batch_z, num_atoms, smoothness=smoothness, atomtypes=atomtypes
        )
        margin = (dists - T).abs()
        ...
        dists = soft_distances(
            atoms, zz, batch_atoms, batch_z, num_atoms, smoothness=smoothness, atomtypes=atomtypes,
        )
...
class dMaSIFConv(nn.Module):
    def __init__(
        self, in_channels=1, out_channels=1, radius=1.0, hidden_units=None
    ):
